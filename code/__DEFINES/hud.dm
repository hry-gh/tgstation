...
#define WIDESCREEN_VIEWPORT_SIZE "26x15"
#define WIDESCREEN_DOCKED_VIEWPORT_SIZE "19x15"
...
// HUD displacement groups: elements in the same group move together when displaced by the chat browser
// Groups should only contain elements that coexist on the same HUD type.

// === Human/living HUD ===

// Right-side status column (EAST-1:28, varying Y)
#define HUD_DISPLACEMENT_GROUP_RIGHT_STATUS list( \
	HUD_MOB_HEALTH, HUD_MOB_HEALTHDOLL, HUD_MOB_STAMINA, HUD_MOB_SPACESUIT, \
	HUD_MOB_MOOD, HUD_MOB_HUNGER, HUD_MOB_BLOOD_LEVEL, HUD_MOB_STYLE_METER)

// Bottom-right controls (EAST-1 to EAST-4, SOUTH to SOUTH+1)
#define HUD_DISPLACEMENT_GROUP_BOTTOM_RIGHT list( \
	HUD_MOB_INTENTS, HUD_MOB_MOVE_INTENT, HUD_MOB_ZONE_SELECTOR, \
	HUD_MOB_THROW, HUD_MOB_CRAFTING_MENU, \
	HUD_MOB_NAVIGATE_MENU, HUD_MOB_MEMORIES, HUD_MOB_PULL, \
	HUD_MOB_AREA_CREATOR, HUD_MOB_COMBO, \
	HUD_MOB_REST, HUD_MOB_SLEEP, HUD_MOB_RESIST, \
	HUD_MOB_FLOOR_CHANGER, HUD_MOB_LANGUAGE_MENU)

// Clothing grid (WEST side)
#define HUD_DISPLACEMENT_GROUP_CLOTHING list( \
	HUD_KEY_ITEM_SLOT(ITEM_SLOT_OCLOTHING), HUD_KEY_ITEM_SLOT(ITEM_SLOT_ICLOTHING), \
...
// Item bar (CENTER, SOUTH)
#define HUD_DISPLACEMENT_GROUP_ITEM_BAR list( \
	HUD_KEY_ITEM_SLOT(ITEM_SLOT_ID), HUD_KEY_ITEM_SLOT(ITEM_SLOT_BELT), \
...
// Alien right-side status (EAST, CENTER varying Y)
#define HUD_DISPLACEMENT_GROUP_ALIEN_STATUS list( \
	HUD_ALIEN_PLASMA_DISPLAY, HUD_ALIEN_QUEEN_FINDER)

// Alien bottom bar (CENTER, SOUTH)
#define HUD_DISPLACEMENT_GROUP_ALIEN_BAR list( \
	HUD_ALIEN_HUNTER_LEAP)

// === Blob HUD ===

// Blob right-side status (EAST-1:28, CENTER)
#define HUD_DISPLACEMENT_GROUP_BLOB_STATUS list( \
	HUD_BLOB_POWER_DISPLAY, HUD_BLOBBERNAUT_OVERMIND, HUD_OOZE_NUTRITION_DISPLAY)

// Blob bottom bar (CENTER, SOUTH)
#define HUD_DISPLACEMENT_GROUP_BLOB_BAR list( \
	HUD_BLOB_BLOBBERNAUT, HUD_BLOB_RESOURCES, HUD_BLOB_NODE, \
	HUD_BLOB_FACTORY, HUD_BLOB_READAPT, HUD_BLOB_RELOCATE, \
	HUD_BLOB_JUMP_TO_CORE, HUD_BLOB_JUMP_TO_NODE)

// === Cyborg HUD ===

// Cyborg bottom bar (CENTER, SOUTH)
#define HUD_DISPLACEMENT_GROUP_CYBORG_BAR list( \
	HUD_CYBORG_HANDS, HUD_CYBORG_DEATH, HUD_SILICON_ALERTS)
...
#define HUD_DISPLACEMENT_GROUP_PAI_CENTER list( \
	HUD_PAI_HOST_MONITOR, HUD_PAI_GPS, HUD_PAI_NEWSCASTER, \
	HUD_AI_CREW_MANIFEST, HUD_SILICON_TABLET)

// Silicon right (EAST area, SOUTH)
#define HUD_DISPLACEMENT_GROUP_SILICON_RIGHT list( \
	HUD_AI_TAKE_IMAGE, HUD_AI_IMAGE_VIEW, HUD_CYBORG_RADIO, \
	HUD_SILICON_TAKE_IMAGE)

// === AI HUD ===

// AI main row (BOTTOM:6, LEFT to RIGHT)
#define HUD_DISPLACEMENT_GROUP_AI_MAIN list( \
	HUD_AI_SENSORS, HUD_AI_CAMERA_LIGHT, HUD_AI_CAMERA_TRACK, \
	HUD_AI_CAMERA_LIST, HUD_AI_IMAGE_VIEW, \
	HUD_AI_CREW_MONITOR, HUD_AI_CREW_MANIFEST, HUD_SILICON_ALERTS, \
	HUD_AI_AICORE, HUD_AI_CALL_SHUTTLE, HUD_AI_ANNOUNCEMENT, \
	HUD_AI_STATE_LAWS, HUD_SILICON_TABLET, \
	HUD_AI_MULTICAM, HUD_AI_ADD_MULTICAM, HUD_AI_TAKE_IMAGE)

// AI floor/z-level cluster (BOTTOM+5, RIGHT)
#define HUD_DISPLACEMENT_GROUP_AI_FLOOR list( \
	HUD_AI_FLOOR_INDICATOR, HUD_AI_GO_UP, HUD_AI_GO_DOWN)

// === Ghost HUD ===

// Extra keys to merge into the ghost type group
#define HUD_DISPLACEMENT_GROUP_GHOST_EXTRAS list( \
	HUD_MOB_LANGUAGE_MENU, HUD_MOB_FLOOR_CHANGER)

// === Guardian HUD ===

// Guardian bottom bar (CENTER, SOUTH)
#define HUD_DISPLACEMENT_GROUP_GUARDIAN_BAR list( \
	HUD_GUARDIAN_COMMUNICATE, HUD_GUARDIAN_MANIFEST, \
	HUD_GUARDIAN_RECALL, HUD_GUARDIAN_TOGGLE, HUD_GUARDIAN_LIGHT)

// === Antag overlays ===

// Changeling status (right side)
#define HUD_DISPLACEMENT_GROUP_CHANGELING list( \
	HUD_CHANGELING_CHEMS, HUD_CHANGELING_STING)

// Voidwalker abilities (CENTER, SOUTH)
#define HUD_DISPLACEMENT_GROUP_VOIDWALKER list( \
	HUD_VOIDWALKER_SPACE_CAMO, HUD_VOIDWALKER_VOID_JUMP)

#define HUD_DISPLACEMENT_KEY_GROUPS list( \
	HUD_DISPLACEMENT_GROUP_RIGHT_STATUS, \
	HUD_DISPLACEMENT_GROUP_BOTTOM_RIGHT, \
	HUD_DISPLACEMENT_GROUP_CLOTHING, \
	HUD_DISPLACEMENT_GROUP_ITEM_BAR, \
	HUD_DISPLACEMENT_GROUP_ALIEN_STATUS, \
	HUD_DISPLACEMENT_GROUP_ALIEN_BAR, \
	HUD_DISPLACEMENT_GROUP_BLOB_STATUS, \
	HUD_DISPLACEMENT_GROUP_BLOB_BAR, \
	HUD_DISPLACEMENT_GROUP_CYBORG_BAR, \
	HUD_DISPLACEMENT_GROUP_PAI_LEFT, \
	HUD_DISPLACEMENT_GROUP_PAI_CENTER, \
	HUD_DISPLACEMENT_GROUP_SILICON_RIGHT, \
	HUD_DISPLACEMENT_GROUP_AI_MAIN, \
	HUD_DISPLACEMENT_GROUP_AI_FLOOR, \
	HUD_DISPLACEMENT_GROUP_GUARDIAN_BAR, \
	HUD_DISPLACEMENT_GROUP_CHANGELING, \
