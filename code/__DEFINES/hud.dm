...
// Key-based groups for elements without a shared parent type
...
// Extra keys to merge into the ghost type group (not /atom/movable/screen/ghost but visually in the same row)
#define HUD_DISPLACEMENT_GROUP_GHOST_EXTRAS list( \
	HUD_MOB_LANGUAGE_MENU, HUD_MOB_FLOOR_CHANGER)

// pAI/silicon bottom-left cluster (WEST area, SOUTH)
...
// pAI/silicon center cluster (CENTER area, SOUTH)
...
// Silicon right cluster (EAST area, SOUTH)
...
// Type-based groups for elements with a shared parent type
